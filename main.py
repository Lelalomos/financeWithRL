from functions import prepare_data_forlstm, detect_outliers_iqr, predict_rsi
...
def main():
    ...
    data_preparing['rsi_pred'] = data_preparing.apply(lambda x :predict_rsi(x[['rsi_14','Close']],rsi_model_path),axis=1)
    
    # stochrsi pred
    stochrsi_model_path = os.path.join(os.getcwd(),'saved_model','stochrsi_14_model.pth')
    data_preparing['stochrsi_pred'] = data_preparing.apply(lambda x :predict_rsi(x[['stochrsi_14','Close']],stochrsi_model_path),axis=1)
